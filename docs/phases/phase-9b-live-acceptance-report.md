...
- 对该条件的离线复现揭示预算观察误停：任务/执行行先于真正启动或请求而入库，
  ...
  完整 live 合同 149/149、lint 通过；提交为 `d3489c0bef6ce1844b087d5a05daeb341bf8a22b`。
- 该提交上的 run `a2965956-c65a-4f47-8010-2fb4f4967497` 已通过真实 SignalR 补拉、
  自动轮换、Device Node 重启 C 处理（1.978 秒）、自动 Conversation 冷启动恢复和
  正常退出；只有一个原生 Task，重启前后 Task/Execution/Thread/Turn 身份不变，
...
| `a10b35b0-ad16-479d-ab78-46f9ed5edd77` | 修后 H/G、即时产物投影及产物冷启动恢复通过；证据仍为 `LIVE_PARTIAL` | 4 / 2 / 0 / 2 |
...
- `62bb130d-e1e6-4968-b0c8-36a506a4a07f` 的证据 SHA 为
  ...
  secrets scan 和离线 E2E 152/152 通过。
- 修复提交 `5d91b96608a158a723d6899841e74e5deb4c8c10` 的 macOS 服务发布（41.59 秒）、
  Desktop 打包（51.09 秒，14/14）、发布后受控 API 启动、归档禁入配置文件检查和
  built-renderer（8.73 秒，观察完整、stderr 为空、所属进程和 profile 清理成功）通过。
- 同一提交上的 run `a10b35b0-ad16-479d-ab78-46f9ed5edd77` 完成 H/G：真实权限请求
  经 Desktop 批准，产物内容和原生文件变更生成的持久化清单一致，UI 即时显示正确 hash；
  重复批准没有改变批准记录，旧批准没有批准另一待处理请求；另一请求经 Desktop 拒绝，
  目标文件未创建，原产物未变化。冷启动后该产物在同一 Conversation 自动恢复，
  下一条消息序号连续，正常退出无 Connected 孤儿。H 30.782 秒、G 11.337 秒、
  冷启动 21.671 秒；没有进程重试。运行清理、认证保留和证据验证通过。
  证据 SHA 为 `511217d0790a7c40b9b5175ab40093667207ab54dffa2c1b62621114b7fbd7b2`。
- 全部补测失败运行均已清理其独立运行目录、保留同一次登录，并通过失败证据包验证。
  最终独立 Standards/Spec 审查已完成，处置见下节；最终候选冻结、最终登录、完整 A–J 和当前 SHA 的 CI 仍待执行。
  最终私有驱动及候选绑定登录控制器已准备并通过无认证夹具检查；未创建最终认证目录。
  最近远端只读核验仍为 PR #8 Draft/Open/unmerged，head `243d6db220ff2b00bcde4fc32df762d8cc7076f5`、
  base `5df7533141107585cfbaa90a9c40d78a7b0b959a`，保留 `full-matrix` 标签；
  Run `34081001060` 的成功仅属于旧远端 SHA，不能移用到本轮代码。当前修复尚未推送。

## 最终独立审查与冻结准备 — 2026-09-12

用户明确允许增加一名 DeepSeek reviewer。该只读审查固定比较
`f681109c0c24c7ba3bff850e10a663df5491e32e` 至
`9c2e1f1841551814bfe407ec05008543065e2f39` 的 19 个路径，并读取四个最终私有控制器源码；
未读取认证材料、运行 live 或修改文件。审查不替代此前已执行的本地测试。

- Standards：没有文档标准违规。两项标为 P2 的重复验证/重复耐久状态读取建议，
  reviewer 明确说明不是当前缺陷、无需冻结前修改；主代理保留严格投影和两次防竞态读取。
- Spec：认证隔离、预算观察顺序、路径 C 的 BLOCKED 均获确认。
  实际缺口是最终打包产物未绑定、失败分类不够具体，以及完整执行与中途停止的区分。
  主代理已修正私有最终驱动：启动前及结束时校验 Desktop、API、Device Node
  的文件树摘要、模式及内部符号链接，并绑定四个控制器源码和冻结 SHA。
  所有禁入文件名与越界链接在读取产物内容前拒绝；不遍历认证目录。
  构建绑定只允许在冻结 SHA、干净工作树下创建一次。
- 证据现在分别保留具体的白名单失败码、合法的顶层错误分类、10 项场景的观察/
  通过/阻断/失败数量及 COMPLETE 或 STOPPED。预算耗尽保留专用状态。
  完整执行且 I 走 C 路径仍为 `LIVE_PARTIAL`，不会把它称为原生恢复 PASS；
  中途失败为 STOPPED，不能当作完整验收。源码或产物变化仍写入失败证据。
- 修改后的无认证夹具已覆盖候选不匹配、产物内容变化、禁入文件、越界链接、
  完整矩阵中的单项阻断、提前停止、预算/源码/清理错误分类。
  输出边界 4/4、官方登录目标自检 9/9 和最终 Desktop 适配器离线 Electron 夹具
  （3.52 秒）通过；未开始最终登录。
  这些处置由主代理验证，没有宣称 reviewer 对修改后私有源码再次独立审查。
- 产品代码及 tracked harness 未因本次审查改变；无需重复此前完整产品回归。
  冻结后必须从该 SHA 重建实际 macOS 服务和 Desktop 包，通过发布后检查，
  再创建构建绑定清单，最后启动唯一一次最终浏览器登录。

登录控制器崩溃不能作为再登录的理由。保留一次性尝试标记及其中所属认证目录；
标记用于阻止重复启动，成功分配文件才是 AUTH_READY 的权威记录。
若控制器中断，先检查所属进程是否仍运行；运行则继续等待。进程已退出时只对
原目录执行静默官方登录状态检查：成功则在同一候选、同一目录恢复元数据分配，
不得重新浏览器登录；检查不可用则保留等待。只有明确取消、明确失败或认证无效
才允许清理所属认证目录。不得通过删除尝试标记自动启动另一轮登录；若确需替换
最终认证，先报告已确认的失败并取得用户明确指示。
