...
固定的真实 Codex `0.146.0` 已完成一次只读、网络关闭、单目录 smoke，返回精确文本 `JARVIS_PHASE4_SMOKE_OK`，输入 SHA-256 与目录项均未变化。此前仅依赖 prompt boundary 的方案已替换为每个 task 的 custom permission profile；本机 macOS probe 已证明 allowed marker 可读，而 sibling marker 与 allowed root 内 `.env` 均返回 `Operation not permitted`。CI 测试不依赖本机 Codex，真实 live auth 仍未验证。
...
- 权限边界：Codex Task 创建时必须携带由 Control Plane 持久化的 capability envelope；领取结果是任务上限、设备注册能力和节点请求能力三者的交集，节点不能扩大 write、command、network 或 allowed roots。缺失 envelope 的 claim 在启动 Codex 前 fail closed 并持久化 `capability_envelope_missing` 的 `task.failed`。Device Node 为每个 task 通过 CLI `-c` 选择 custom permission profile，以原生 filesystem/network policy 和敏感路径 deny 实现边界，并验证 app-server 返回的 active profile。
...
- 诊断安全：Codex stderr 内容不再写入事件或日志，只保留“已脱敏”的存在性摘要；子进程仅继承 `PATH`、`LANG` 和显式安全环境项，并使用独立、owner-only 的 `CODEX_HOME`，不读取用户现有 `~/.codex`。
...
- 真实 live auth 尚未验证；本机 probe 只证明固定 Codex `0.146.0` 的 custom permission profile 在当前 macOS 账户下的文件边界行为，不能替代生产账户、发布包和多机验收。
